...
11. Node.js
12. LLM Access
13. The Database Proxy
14. Building MCP Servers
15. ShoeString in Action
16. The Designer
...
The minimal application consists of 3 files : 
- a unit - usually a form component for web apps or a node.js init unit.
- an entrypoint so the compiler knows which files to include
- a html file for web-apps.
...
the entrypoint specification, with at least 1 form
...
and a simple html file which executes the compiled js file
...
<meta name="description" content="lynkfs web design services">
<meta name="apple-mobile-web-app-status-bar-style" content="default">
  <title>ShoeStringV2</title>
...
One Pascal file, One entrypoint definition, One HTML file, One compiled JavaScript file. No build system, no package manager, no bundler.
...
The kitchen-sink app (`forms/Kitchensink.pas`) ships a live demo for every widget below — pick a name from its listbox to see source and runtime behaviour side-by-side. The richer widgets (Tree View, Data Grid, Chat Panel) get full treatment further down; the rest are catalogued first.

## Display

**Label** — `JW3Label`. Plain text. `Caption` property; optional `ForElement` to associate with an input for accessibility.

**Badge** — `JW3Badge`. Small pill for status. Variants via class: `csBadgePrimary`, `csBadgeSuccess`, `csBadgeWarning`, `csBadgeDanger`, `csBadgeInfo`.
```pascal
var B := JW3Badge.Create(Row);
B.Caption := 'Active';
B.AddClass(csBadgeSuccess);
```

**Image** — `JW3Image`. Wraps `<img>` with `Src`, `Alt`, and a `Fit` enum (`ifContain` / `ifCover` / `ifFill` / `ifNone`) that maps to `object-fit`.

**Spinner** — `JW3Spinner`. Three-dot loading indicator. Size, colour and speed via `--spinner-size`, `--spinner-color`, `--spinner-speed`.

## Buttons and Form Fields

The `.field` base class (Chapter 7) gives every form widget consistent height, padding, focus ring, and invalid state. Form-ulator (Chapter 7) wires these declaratively for routine forms.

**Button** — `JW3Button`. `Caption` plus `OnClick`. Variants: `csBtnPrimary`, `csBtnSecondary`, `csBtnDanger`, `csBtnGhost`. Sizes: `csBtnSmall`, `csBtnLarge`.
```pascal
var Save := JW3Button.Create(Bar);
Save.Caption := 'Save';
Save.AddClass(csBtnPrimary);
Save.OnClick := HandleSave;
```

**Input** — `JW3Input`. `Value`, `Placeholder`, `InputType` (`text` / `email` / `password` / `number` / `tel` / `url` / `date` / `time` / `color` / …), `OnChange` (fires on every keystroke), `ReadOnly`.

**TextArea** — `JW3TextArea`. Multi-line `<textarea>`. `Value`, `Placeholder`, `Rows`, `OnChange`, `ReadOnly`.

**Select** — `JW3Select`. `<select>` dropdown. `AddOption(value, text)`, `Value`, `SelectedIndex`, `SelectedText`, `OnChange`, `ClearOptions`.

**Checkbox** — `JW3Checkbox`. `Checked`, `Caption`, `Enabled`, `OnChange(Sender, Checked: Boolean)`.

**Switch** — `JW3Switch`. iOS-style toggle. Same shape as Checkbox: `Checked`, `Caption`, `Enabled`, `OnChange`. `Value` is provided as an alias for `Checked`.

**Radio Group** — `JW3RadioGroup`. `AddButton(caption, value)` adds an option; `SelectedIndex`, `SelectedValue`, `Enabled`, `OnChange(Sender, ItemIndex, Value)` for reads and notifications.

## Containers

**Panel** — `JW3Panel`. Generic flex container — the building block for both layout patterns and form rows. Use `SetGrow(1)` for fill-the-rest behaviour and `SetStyle('flex-direction', 'row')` to switch from column to row.

**Card** — `JW3Card`. Bordered surface with `Header`, `Body`, `Footer` sub-panels and a top-level `Title` property. Radius, padding, and shadow are theme-token-driven.

**Product Card** — `TProductCard`. Card specialized for commerce — `Title`, `Price`, `Description`, `ImageSrc`, plus `AddTag(text)` / `ClearTags`. Intended as a starting point, not a finished shop component.

**Tabs** — `JW3TabControl`. `AddTab(caption)` returns the tab's content panel — populate it like any other parent. `ActiveIndex`, `OnTabChanged(Sender, Index)`, `RemoveTab(index)`, `Panel(index)`.

**Toolbar** — `JW3Toolbar`. Horizontal strip. `AddItem(caption)` for clickable items, `AddSpacer` to flex-grow gaps, `AddSeparator` for dividers. Each item's `OnClick` is wired by the caller.

**Drawer** — `JW3Drawer`. Slide-in panel from the side, with `Header` / `Body` / `Footer` sub-panels and a built-in close button. `Open`, `Close`, `Toggle`, `IsOpen`. Backdrop dims the rest of the page while open; transition duration tunable via `SetDuration`.

## Overlays

**Modal** — `JW3Modal`. Dialog with backdrop. `Show` / `Hide`, exposes `Body` and `Footer` panels for the content. `OnClose` fires on backdrop click or close button. Title via `Title` property.

**Toast** — a top-level procedure, not a class:
```pascal
Toast('Saved.', ttSuccess, 4000);
```
Types: `ttInfo`, `ttSuccess`, `ttWarning`, `ttDanger`. Dismisses after `DurationMs` (default 3000ms). Stacks if multiple are queued. No setup — call from anywhere.

## Data Display

**ListBox** — `JW3ListBox`. Single-column selectable list. `AddItem(value, text)`, `Clear`, `Sort([descending])`, `SelectedIndex`, `SelectedValue`, `OnSelect(Sender, Value)`. Used as the navigation sidebar in the kitchen-sink itself.

**Table** — `JW3Table`. Light-weight `<table>` for small datasets. `AddColumn(header, field)` declares columns once; `SetRows(variant)` replaces the body from a row array; `IDField` names the row-id field; `OnDelete(id)` wires per-row deletion. For 1,000+ rows, jump to **Data Grid** below.

## Visualization

**Chart** — `JW3Chart`. SVG-based, four types: `ctBar`, `ctLine`, `ctPie`, `ctDonut`.
```pascal
var C := JW3Chart.Create(Panel);
C.NewSeries('Engineering').AddValue(42);
C.NewSeries('Marketing').AddValue(23);
C.Refresh;
```
`Refresh` re-renders the SVG. Colours auto-cycle through the theme palette unless set per-series.

**Semantic Zoom** — `JW3SemanticZoom`. Multi-level zoomable form for hierarchical data — a single visual that exposes summary, detail, and edit views as the user "zooms in". Specialty component; see the Semantic Zoom kitchen-sink entry for usage. If you don't know whether you need it, you don't.
...
Chapter 11 showed the Node target. Here is the first thing worth building on it: one typed Pascal surface for every LLM, designed to survive constant change.

## The Problem

There are many models — Claude, OpenAI, DeepSeek, Ollama — and Claude alone is reachable three ways: the Messages API, the local `claude` CLI, and managed agents. Each has its own wire format, auth, streaming dialect, and session model. A ShoeString app should see *one* thing, and adding a provider next month should not touch the app, the form, or — given the IDE-only Pascal compiler — recompiled Pascal at all.

## The Shape

Two layers, one rule each.

- **Adapter (Pascal, thin).** A typed client. Holds the conversation and the capability matrix. Holds no keys and no wire knowledge. `Send` / `SendStreaming`; swapping `TOpenAIAdapter` for `TClaudeAdapter` changes nothing else.
- **Proxy (ShoeString → Node, stateless).** Owns keys, routes by provider, frames SSE. Knows no wire format itself — each provider is a runtime-loaded JS plugin named in `providers.json`.

The split is deliberate. The adapter surface is stable, so it is Pascal. Provider wire formats are the churn, so they are data + JS plugins the compiled skeleton loads at runtime. Adding a provider is a JSON row and a small `.js` — **never a Quartex recompile**. This is the whole future-proofing argument: put the part that changes weekly where it can change without the GUI compiler.

```
app ─► TLLMAdapter ──POST /v1/chat──► JLLMProxy.pas ──► plugins/<provider>.js ─► model
       (typed, no keys)  (stable skeleton, stateless)   (the churn, edit-and-run)
```

## Multi-turn is the Shape, not a Feature

The canonical request is always a `messages` array. A single prompt is the one-element case. The adapter appends the user turn, appends the assistant turn on success, pops the user turn on failure — so a follow-up *is* just another `Send`. Nothing to enable.

State stays out of the proxy. The client sends the full history every turn; provider-native sessions (Claude CLI `--resume`, managed agents) ride an opaque `ProviderSessionId` the proxy passes straight through and never interprets. A stateless proxy restarts safely and scales sideways — the same lesson as the database proxy.

## Streaming

`SendStreaming` returns tokens as they arrive. The proxy normalises every provider's stream into one event vocabulary — `delta`, `done`, `error` — and the plugin owns the upstream stream; the skeleton owns only the SSE frame. The UI never sees a provider difference. Piping a turn into a `JW3DataStore` key (Chapter 10) is one call, and the chat panel just subscribes — the transport and the view never meet.

## Capabilities, honestly

Each adapter reports what it can do. Web search is enabled only where a provider supports it natively (the Claude API attaches Anthropic's `web_search` tool); where it is absent the result says so via `webSearchUnavailable` and no fallback searcher is invented. Reporting an absent capability beats faking it.
...
One language end to end costs a Quartex compile of the skeleton. The design confines that cost to the parts that almost never change and pushes everything volatile into data and plugins. If you only ever call one provider, the plain reference proxy is simpler; the moment "many providers, many changes" is the brief, this is the shape that does not fight you.

# Chapter 13: Database Connectivity

In a client–server app the client expresses *intent*; the server turns it into SQL. The goal is a single pattern that covers **any** SQL statement — DDL, and queries with dynamic identifiers (table names, column names, sort keys) — with zero injection surface.

## Options

**1. Prepared statements.** Parameterised string plus a values array; the driver binds. Placeholders bind *values* only — `ORDER BY ?` or `DROP TABLE ?` is not valid SQL. The moment an identifier is dynamic, injection is back in the string.

**2. Structured intent / AST.** The client sends JSON describing the operation; the server owns all SQL generation and validates every identifier against an allowlist. Safe, but the server now carries per-app table and column allowlists — adding a table is a server change.

**3. SQL → AST → SQL.** Parse readable SQL to an AST on the client, ship the AST, reconstruct on the server. The developer writes normal SQL; the wire format is structured. Still needs server-side allowlist validation — calling `sqlify()` on an untrusted AST is as dangerous as accepting raw SQL (cf. CVE-2026-28399). Safety lives in the validator, not the parser.

**4. Template plus identifier slots.** `?` for values, `{{table}}` for identifiers; the server substitutes identifiers after allowlist checks. Feels like SQL, no client library — but subqueries and CASE get awkward, and the server still owns allowlists.

**5. Schema-blind proxy.** The client sends plain parameterised SQL and a values array. The server is a thin authenticated relay: verify a JWT (`sub, app, role`), map `app + role` to a database role from a static config, `SET ROLE` on the connection, run the parameterised query, reset the role, release the connection. Security is enforced **entirely by database GRANTs**. The server holds zero table names, zero column names, zero allowlists. A new app or table is a `GRANT` plus one config line — never a code change.

## Schema-blind server architecture

Shoestring-V2 has implemented the schema-blind architecture. The basic reason is that it follows the "if the browser does it, don't" paradigm. In this case "if the database does it, don't"

There is a separate doc [lynkfs.com/docs/security](https://lynkfs.com/docs/security) which describes this in detail.

## Security

The same doc [lynkfs.com/docs/security](https://lynkfs.com/docs/security) also describes security issues and measures for client server web apps.

## Authentication

Also maybe of interest is a separate doc on authentication [lynkfs.com/docs/authentication](https://lynkfs.com/docs/authentication). Shoestring-v2 has implemented passkeys as a preferred method.

# Chapter 14: Building MCP Servers

The LLM chapter consumed tools — native web search where it exists. This chapter builds them: a Model Context Protocol server in Object Pascal, the same way and with the same restraint as everything else here.

## The Problem

An LLM becomes useful when it can call out — query a database, fetch a URL, run a calculation. MCP is the protocol for that. The reference servers reach for the official SDK; a ShoeString server should not. The protocol's stdio transport is newline-delimited JSON-RPC 2.0 — small enough to implement directly, which keeps the dependency count at zero and the `asm` confined to the wire, exactly as the Node HTTP server and the proxy do.

## The Scaffold

`JMcpServer.pas` is the whole framework: a registry and a stdin loop.

```pascal
uses JMcpServer;

RegisterTool('http_get',
  'GET a URL, return up to 4000 chars.',
  ...
  begin
    ...
    if url = '' then Reject('Missing "url".')
    else
      asm
        (async function () {
          try { var r = await fetch(@url); (@Resolve)((await r.text()).slice(0,4000)); }
          catch (e) { (@Reject)('fetch: ' + (e && e.message)); }
        })();
      end;
  end);
...
It answers `initialize`, ignores `notifications/initialized`, emits each tool's JSON schema for `tools/list`, dispatches `tools/call`, and returns the right JSON-RPC error for an unknown method or tool. A handler that throws becomes a tool result with `isError: true` — a tool failing is data, not a crash.

## Sync or async, same handler

A handler gets `Resolve` and `Reject`. Call them now (echo, arithmetic) or in a `fetch`/database/child-process callback minutes later — the scaffold writes the JSON-RPC reply *when they fire*, not when the handler returns. No async on methods, no promises in the signature; the closure-parameter-in-`asm` idiom is the same one the JPromise executor uses (Chapter 2's async note, made concrete). A database tool is a handler that `require`s its driver and resolves with rows — the scaffold neither knows nor cares.

## The one invariant

Stdout is JSON-RPC and nothing else. A stray `console.log` is a corrupt frame and a dead server. Diagnostics go to stderr; `StartMcp` logs its banner there. The scaffold writes only responses to stdout — hold any tool you add to the same line.

## Symmetry

Onboarding a tool to `JMcpServer` is what onboarding a provider is to the LLM proxy: one contained edit at the edge, the core compiled once and untouched. The same shape keeps recurring in this framework because it keeps being the cheap one — a thin typed Pascal spine, the volatile detail pushed to a registered edge, `asm` only where Pascal cannot reach the runtime. If a server here grows a second concept, that is the signal to stop and look, not to add a third.

# Chapter 15: ShoeString in Action
