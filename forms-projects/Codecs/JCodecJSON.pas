...
function JW3CodecJSON.Encode(Input: variant): JPromise;
var
  Indent: Integer;
begin
  Indent := FIndent;
  // Sync work, async return — the degenerate case of the Phase-2 contract.
  asm
    try {
      var s = (@Indent > 0)
        ? JSON.stringify(@Input, null, @Indent)
        : JSON.stringify(@Input);
      @Result = Promise.resolve(s);
    } catch (e) {
      @Result = Promise.reject(new Error('JSON encode failed: ' + (e && e.message ? e.message : e)));
    }
  end;
end;

function JW3CodecJSON.Decode(Input: variant): JPromise;
begin
  asm
    try {
      @Result = Promise.resolve(JSON.parse(@Input));
    } catch (e) {
      @Result = Promise.reject(new Error('JSON decode failed: ' + (e && e.message ? e.message : e)));
    }
  end;
end;
